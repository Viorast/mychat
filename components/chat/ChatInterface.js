...
export default function ChatInterface({ chatId }) {
    ...
    /**
     * Handle message edit - Regenerate AI response after edit
     */
    const handleEditMessage = async (messageId, newContent) => {
        try {
            await editMessage(messageId, newContent);
            // Note: The AI response regeneration would require additional backend logic
    ...
    const handleDrop = (e) => {
        ...
        if (!hasActiveChat || isStreaming) return;

        const files = e.dataTransfer.files;
        if (files && files.length > 0) {
            const file = files[0];
            if (file.type.startsWith('image/')) {
                // Call the exposed function in MessageInput
                if (typeof window !== 'undefined' && window.handleImageDrop) {
            ...
            {isDragging && (
                <div className="absolute inset-0 bg-blue-500/20 backdrop-blur-sm z-50 flex items-center justify-center pointer-events-none">
                    <div className="bg-white rounded-2xl p-8 shadow-2xl text-center">
                        ...
                        <p className="text-xl font-semibold text-gray-800">Drop image here</p>
