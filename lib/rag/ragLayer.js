...
STEP 2: If TRUE, classify analysis type
- "What/How many" questions → "descriptive"
- "Why/What caused" questions → "diagnostic"

STEP 3: If TRUE, generate ONE valid PostgreSQL SELECT query

CRITICAL SQL RULES:
- Always quote: "SDA"."table_name"
- Add LIMIT 100
- Use JOINs for multi-entity
- Use WHERE for filtering
- Use GROUP BY for aggregations

UNDERSTANDING NEGATIONS (IMPORTANT):
If user asks about X but defines opposite Y, query for NOT Y.
Examples:
- "Who is LATE? (on-time = check_in < '09:00')" → Query WHERE check_in >= '09:00:00'
- "Which are SLOW? (fast = duration < 2hrs)" → Query WHERE duration >= 2

Primary question ALWAYS takes precedence over definitions!

OUTPUT (JSON only, no explanations):
{
  "requiresRetrieval": true/false,
  "analysisType": "descriptive"/"diagnostic"/"none",
  "query": "SELECT ... FROM \\"SDA\\".\\"table\\" ... LIMIT 100" or null
}
`;
    ...
    generateFinalResponse = async (userMessage, history, formattedData, analysisType, image = null) => {
        ...
        DATA VISUALIZATION (Gunakan jika data NUMERIK dan cocok untuk grafik):
        Jika user meminta visualisasi, chart, grafik, atau jika data cocok untuk divisualisasikan (perbandingan angka, distribusi, trend), sertakan tag CHART setelah penjelasan teks.
