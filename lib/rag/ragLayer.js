...
const MAX_CONTEXT_CHUNKS = 3; // ✅ OPTIMIZED: Reduced from 5 to 3
...
class RagLayer {
    ...
    /**
     * ✅ OPTIMIZED: Langkah 2: Reranking with score-based approach
     */
    rerankContext = async (userMessage, history, retrievedChunks) => {
        ...
        // Use optimized score-based reranking (saves ~2,400 tokens, ~27 seconds)
        if (USE_OPTIMIZED_RERANKING) {
            const result = rerankContextOptimized(userMessage, retrievedChunks);
            console.log(`[RAG Tokens] Reranker Usage (Optimized): ${JSON.stringify(result.usage)}`);
            return result;
        }

        // Fallback to LLM-based reranking (original code)
        console.log(`[RAG v1.4] ⚠️  Using LLM-based reranking (optimization disabled)`);
    ...
    generateSQLPlan = async (userMessage, history, rerankedContext) => {
        ...
        3. Jika 'requiresRetrieval = true', buat **SATU query SQL SELECT** yang valid untuk PostgreSQL.
           ...
           **ATURAN WAJIB untuk SQL Query:**
           - SELALU gunakan double quotes untuk schema dan tabel: "SDA"."nama_tabel"
           - SELALU gunakan double quotes untuk nama kolom jika perlu
           - Contoh BENAR: SELECT * FROM "SDA"."log_absen"
           - Contoh SALAH: SELECT * FROM SDA.log_absen (JANGAN seperti ini!)
    ...
    processQuery = async (userMessage, history, image = null) => {
        ...
        try {
            ...
            if (intent === "general_conversation") {
                ...
                const rerankResult = await this.rerankContext(userMessage, limitedHistory, retrievalResult.results);
