...
class RagLayer {
    ...
    retrieveContext = async (userMessage) => {
        ...
        try {
            // ✅ QDRANT: Intelligent collection routing
            const query = userMessage.toLowerCase();
            let collections = [];

            // Detect query intent for collection routing
            // ✅ FIX: Use \b word boundaries on short words to prevent false substring matches
            // ✅ FIX: Check isPolicy FIRST — it's the most specific intent
            const isPolicy = /kebijakan|policy|sop|aturan|peraturan|boleh|diperbolehkan|perusahaan|company/i.test(query);
            const isPrediction = /prediksi|forecast|estimasi|proyeksi|next month|bulan depan|\bakan\b|trend ke depan/i.test(query);
            const isAnalysis = /trend|pola|pattern|analisis|analysis|rata-rata|average|insight/i.test(query);
            const isSQL = /siapa|berapa|list|daftar|data|show|tampilkan/i.test(query);

            if (isPolicy) {
                // Policy queries → company knowledge (highest priority)
                collections = ['company_knowledge'];
                console.log('[RAG] Route: Policy query → knowledge');
            } else if (isPrediction) {
                // Prediction queries → insights + schema
                collections = ['aggregated_insights', 'schema_context'];
                console.log('[RAG] Route: Prediction query → insights + schema');
            } else if (isAnalysis) {
                // Analysis queries → insights + schema
                collections = ['schema_context', 'aggregated_insights'];
                console.log('[RAG] Route: Analysis query → schema + insights');
            } else if (isSQL) {
                // SQL/detail queries → schema only
                collections = ['schema_context'];
                console.log('[RAG] Route: SQL query → schema');
            } else {
                // Fallback → all collections
                collections = ['schema_context', 'aggregated_insights', 'company_knowledge'];
                console.log('[RAG] Route: General query → all collections');
            }
