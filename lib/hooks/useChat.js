...
export function useChat() {
  ...
  const sendMessage = useCallback(async (messageData) => {
    ...
    // Bisa kirim jika ada teks ATAU gambar
    ...
    const userMessagePayload = {
        ...
        image: imageContent // Simpan data gambar di pesan pengguna (untuk display)
    };
    ...
    try {
        const response = await fetch('/api/chat', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ message: textContent,image: imageContent, chatId: currentChatId }),
        });

        if (!response.ok) { // Periksa status HTTP
        ...
        setError(err.message); // Set error global
        // Update pesan AI dengan pesan error
        updateMessage(assistantMessageId, { content: 'Error: ' + err.message, isStreaming: false, isError: true });
    } finally {
        setStreaming(false); // Pastikan streaming di-set false
        // Muat ulang daftar chat untuk update judul (jika berubah)
        await loadChats();
    }
    
    return { ...context, loadChats, createChat, selectChat, sendMessage };
  }, [addMessage, updateMessage, setError, clearError, setStreaming, loadChats, createChat, activeChatRef]);
...
const updateChatTitle = useCallback(async (chatId, newTitle) => {
    ...
    return {
      ...
      // canSendMessage disesuaikan di MessageInput berdasarkan state lokal & props
