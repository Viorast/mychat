...
// Fallback User UUID (for unauthenticated or loading state)
...
export function useChat() {
  ...
  }, [userId, setLoading, setChats, setError, clearError]);

  // Load chats when user changes or on initial load
  useEffect(() => {
    // Only load when session is ready (not loading)
    if (status === 'authenticated' || status === 'unauthenticated') {
      loadChats();
    }
  }, [userId, status, loadChats]);
  ...
  }, [userId, setLoading, setError, clearError, loadChats, setActiveChat, setMessages, router]);
  ...
  const selectChat = useCallback(async (chat) => {
    if (!chat || !chat.id) {
      console.warn('selectChat called with invalid chat object:', chat);
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        // Silently handle 404 (chat might be deleted) via UI state rather than crashing flow
  ...
  const editMessage = useCallback(async (messageId, newContent) => {
    ...
    try {
      ...
      // Step 2: (SKIPPED) Do NOT reload messages here. Trust local state to avoid flickering.
      // We already updated UI in Step 1.

      // Step 3: If messages were truncated, regenerate AI response
      if (data.shouldRegenerate || data.truncated) {
        ...
        if (aiResponse.ok && aiResponse.body) {
          ...
          const assistantMessageId = `asst-regen-${Date.now()}`;
      ...
      // Step 4: Reload messages to get correct IDs from backend ONLY after everything is done
      try {
        const chatResponse = await fetch(`/api/chat/${currentChatId}`);
        const chatData = await chatResponse.json();
        if (chatData.success && chatData.data && chatData.data.messages) {
          // Merging strategy could be safer, but replacement is okay if backend is truth
          setMessages(chatData.data.messages);
        }
      } catch (e) {
        console.warn('Failed to reload messages after edit:', e);
      }

      await loadChats(); // Update sidebar list
      ...
      // Reload messages in case of error to restore valid state
      if (currentChatId) {
        fetch(`/api/chat/${currentChatId}`).then(r => r.json()).then(d => {
