...
class QueryService {
  ...
  async generateSQLQuery(userPrompt, schema, history = [], additionalContext = {}) {
      ...
      try {
        ...
        console.log('Sending enhanced prompt to Gemini AI with history and context...');

        const response = await this.client.generateResponse(userPrompt, systemPrompt);

        if (!response.success) {
          console.warn('Gemini response not successful, using demo response');
          return this.getDemoResponse(userPrompt);
        }
  ...
  buildSystemPrompt(schema, userPrompt, history = [], additionalContext = {}) {
    ...
    if (additionalContext.rowCounts) {
        additionalContextText += '\n# JUMLAH DATA DALAM TABEL\n';
        additionalContextText += '```json\n' + JSON.stringify(additionalContext.rowCounts, null, 2) + '\n```\n';
    }
    if (additionalContext.sampleData && Object.keys(additionalContext.sampleData).length > 0) {
        additionalContextText += '\n# CONTOH DATA DARI BEBERAPA TABEL\n';
        additionalContextText += '```json\n' + JSON.stringify(additionalContext.sampleData, null, 2) + '\n```\n';
    }
...
Berikut adalah percakapan sebelumnya. Gunakan ini untuk memahami konteks pertanyaan lanjutan.
\`\`\`
${historyText}
\`\`\`
...
4.  **INGAT PERCAKAPAN**: Jika pertanyaan user merupakan kelanjutan dari pertanyaan sebelumnya, gunakan riwayat percakapan untuk memberikan jawaban yang relevan.

# ATURAN PEMBUATAN QUERY SQL
1.  **READ-ONLY**: Hanya dan HANYA \`SELECT\`. Jangan pernah membuat query \`UPDATE\`, \`INSERT\`, \`DELETE\`, \`DROP\`, dll.
2.  **KEAMANAN**: Selalu gunakan \`ILIKE\` untuk pencarian teks agar tidak case-sensitive.
...
### 2. Pertanyaan Lanjutan (Menggunakan Konteks History)
    - **History**: user: "tampilkan pelanggan dari jakarta", assistant: (menampilkan 5 pelanggan dari jakarta)
    - **User Prompt**: "tampilkan 5 lagi"
    - **JSON Respons Anda**:
    \`\`\`json
    {
        "status": "success",
        "query": "SELECT customer_name, address, status FROM \\"SDA\\".\\"customers\\" WHERE address ILIKE '%jakarta%' OFFSET 5 LIMIT 5;",
        "response_type": "analysis",
        "text_template": "Tentu, berikut adalah 5 data pelanggan berikutnya dari Jakarta:"
    }
    \`\`\`

### 3. Jika Pertanyaan di Luar Konteks (Tidak Sesuai Tema)
