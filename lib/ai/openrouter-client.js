...
class OpenRouterClient {
    constructor() {
        // Bootstrap dari .env saat startup
        ...
        if (this.isDemoMode) {
            console.log('OpenRouter running in DEMO mode - Add OPENROUTER_API_KEY in Settings to enable real AI');
        } else {
            console.log(`OpenRouter initialized with model: ${this.model}`);
        }
    }

    /** Ambil config terbaru dari settingsService (dengan fallback ke .env) */
    async _getConfig() {
        const settings = await settingsService.getAll();
        const apiKey = (settings.openrouter_api_key && settings.openrouter_api_key.trim())
            ? settings.openrouter_api_key.trim()
            : this.apiKey;
        const model = (settings.openrouter_model && settings.openrouter_model.trim())
            ? settings.openrouter_model.trim()
            : this.model;
        const visionModel = (settings.openrouter_vision_model && settings.openrouter_vision_model.trim())
            ? settings.openrouter_vision_model.trim()
            : this.visionModel;
        const isDemoMode = !apiKey;

        // Buat OpenRouter client dengan key terbaru
        let client = null;
        if (!isDemoMode) {
            const { OpenRouter: OR } = await import('@openrouter/sdk');
            client = new OR({
                apiKey,
                defaultHeaders: {
        ...
        return { apiKey, model, visionModel, isDemoMode, client };
    }
    ...
    async generateResponse(prompt, context = '', image = null) {
        const { apiKey, model, visionModel, isDemoMode } = await this._getConfig();
        if (isDemoMode) {
        ...
        try {
            ...
            const selectedModel = (image && image.base64) ? visionModel : model;
            if (selectedModel !== model) {
                console.log(`[OpenRouter] Switching to vision model: ${selectedModel} (original: ${model})`);
            }
            ...
            try {
                const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
                    ...
                    // No AbortController here — ai-router.js withTimeout(20s) handles cancellation
                });
                ...
                // Extract response — DeepSeek R1 may put the answer in 'reasoning' with empty 'content'
                const messageObj = completion.choices?.[0]?.message;
                let responseText = messageObj?.content || '';

                // ✅ FIX: DeepSeek R1 reasoning model fallback
                if ((!responseText || responseText.trim() === '') && messageObj?.reasoning) {
                    console.log('[OpenRouter] Content is empty, using reasoning field as fallback (DeepSeek R1 behavior)');
                    responseText = messageObj.reasoning;
                }
                ...
                return {
                    ...
                    usage: completion.usage ? {
                        ...
                        isEstimated: false
                    } : { isEstimated: true, totalTokenCount: 0 }
                };
            ...
            else if (errorMessage.includes('Image ') || errorMessage.includes('gambar') || errorMessage.includes('MimeType')) {
    ...
    async generateStream(prompt, context = '', image = null) {
        const { apiKey, model, visionModel, isDemoMode, client } = await this._getConfig();
        if (isDemoMode) {
        ...
        try {
            ...
            const streamPromise = client.chat.send({
