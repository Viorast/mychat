...
class GeminiClient {
    ...
    /** Ambil config terbaru dari settingsService */
    async _getConfig() {
        const settings = await settingsService.getAll();
        const apiKey = (settings.gemini_api_key && settings.gemini_api_key.trim())
            ? settings.gemini_api_key.trim()
            : this.apiKey;
        const model = (settings.gemini_model && settings.gemini_model.trim())
            ? settings.gemini_model.trim()
            : this.model;
        const isDemoMode = !apiKey;
        let genAI = null;
        if (!isDemoMode) genAI = new GoogleGenerativeAI(apiKey);
        return { apiKey, model, isDemoMode, genAI };
    }
    ...
    async generateResponse(prompt, context = '', image = null) {
        const { isDemoMode, model, genAI } = await this._getConfig();
        if (isDemoMode) {
            return {
                success: false,
    ...
    async generateStream(prompt, context = '', image = null) {
        ...
        try {
            ...
            console.log(`[GeminiFallback] Starting stream with ${model}...`);
