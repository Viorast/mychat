// lib/database/schemaService.js - ENHANCED VERSION
...
class SchemaService {
  ...
  async getFullSchema() {
    ...
    try {
      ...
      // Enhanced schema query dengan lebih banyak detail
      const schemaQuery = `
        SELECT 
          c.table_schema,
          c.table_name,
    ...
    try {
      ...
      const tableCountQuery = `
        SELECT 
          table_schema,
          table_name,
          (xpath('/row/cnt/text()', query_to_xml_format('SELECT COUNT(*) as cnt FROM ' || quote_ident(table_schema) || '.' || quote_ident(table_name), false, true, '')))[1]::text::int as row_count
        FROM information_schema.tables 
        WHERE table_schema IN ('public', 'SDA')
        AND table_type = 'BASE TABLE'
        ORDER BY table_schema, table_name;
      `;

      const result = await this.db.query(tableCountQuery);
      
      const counts = {};
      result.rows.forEach(row => {
        const fullTableName = `"${row.table_schema}"."${row.table_name}"`;
        counts[fullTableName] = row.row_count;
      });
  ...
  async getSampleData(tableName, limit = 3) {
    try {
      ...
      // Extract schema and table name from quoted string
