...
class QueryExecutor {
  ...
  async executeQuery(sqlQuery) {
    ...
    try {
      // Set search_path berdasarkan active_database_context setting
      const activeCtx = await settingsService.getSetting('active_database_context').catch(() => 'SDA') || 'SDA';
      const searchPath = activeCtx === 'DVO'
        ? 'SET search_path TO "public", "DVO", "SDA"'
        : 'SET search_path TO "public", "SDA"';
      await this.db.query(searchPath);

      console.log(`[QueryExecutor] search_path: ${activeCtx}, SQL: ${sqlQuery.substring(0, 80)}...`);
      ...
      if (process.env.RAG_DEBUG === 'true') {
        console.error('[DB DEBUG] === SQL Error Details ===');
        console.error('[DB DEBUG] Error Message:', error.message);
        console.error('[DB DEBUG] Error Code:', error.code);
        console.error('[DB DEBUG] Error Detail:', error.detail);
        console.error('[DB DEBUG] Failed Query (first 300 chars):', sqlQuery.substring(0, 300));
        console.error('[DB DEBUG] === End Error Details ===');
      }
